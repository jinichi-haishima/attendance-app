...
class UpdateAttendanceRecordRequest extends FormRequest
{
    ...
    {
        $attendanceRecordId = $this->route('attendanceRecord');

        return [
            'user_id' => 'required|integer|exists:users,id',
            'date'    => [
                Rule::unique('attendance_records', 'punch_in_time')->ignore($attendanceRecordId)->where(function ($query) {
                    return $query->where('user_id',$this->user_id)->whereDate('punch_in_time', $this->date);
                })
            ],
            'clock_in' => [
                'required',
