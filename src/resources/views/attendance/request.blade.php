            ...
            <table class="attendance-table">
                ...
                <tbody>
                    ...
                    <tr class="attendance-table-row">
                        ...
                        @can('admin-only')
                            <td class="attendance-table-td">
                                <a href="{{ route('admin.attendance.approval', ['attendance_correct_request_id' => $request->id,'date' => $request->punch_in_time ? \Carbon\Carbon::parse($request->punch_in_time)->format('Y-m-d') : '']) }}" class="detail-link">詳細</a>
                            </td>
                        @else
                            <td class="attendance-table-td">
                                <a href="{{ route('attendance-records.detail', ['id' => $request->id,'date' => $request->punch_in_time ? \Carbon\Carbon::parse($request->punch_in_time)->format('Y-m-d') : '']) }}" class="detail-link" >詳細</a>
                            </td>
                        @endcan
                    </tr>
                    @endforeach
